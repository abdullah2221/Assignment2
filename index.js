...
const calculateDiscountUsingIFelse = (price) => {
    let discount;
    if (price > 100) {
...
//By Switch Statment
const suggestClothing = (temperature) => {
    let suggestion;
    switch (true) {
        case (temperature <= 0):
...
const checkNumber = (num) => {
    if (num % 3 == 0 || num % 5 == 0) {
        ...
        console.log(`The number is ${num}, it is neither divisible`);
    }
};
checkNumber(24);
...
const determineDayOfWeek = (dayNumber) => {
    let dayName;
    if (dayNumber === 1) {
        dayName = "Sunday";
    }
...
const calculateBill = (unitConsumed, unitPrice) => {
    let totalAmount;
    if (unitConsumed <= 100) {
